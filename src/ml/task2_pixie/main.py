...
from ml.task2_pixie.graph import UserItemNetwork, pixie_recommend
...
def main():
    ...
    parser.add_argument("--lambdaB", default=1)
    ...
    def _one_user_pixie(u):
        recs = pixie_recommend(
            ...
            alpha=args.alpha,
            topk=args.topk,
            lambda_S=args.lambdaS,
            lambda_B=args.lambdaB,
        )
        return u, recs

    results = Parallel(n_jobs=20)(
    delayed(_one_user_pixie)(u) for u in val_users)

    predictions = {u: recs for (u, recs) in results}
