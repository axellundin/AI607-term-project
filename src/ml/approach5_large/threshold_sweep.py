...
from ml.approach5_large.test import predict_with_threshold
...
def grid_search_individual_thresholds(use_training_set=False, threshold_range=None, step=0.05):
    ...
    Args:
        ...
        threshold_range: Tuple (min, max) for threshold range. If None, uses (0.1, 0.95)
    ...
    print("Loading model for Approach 5 Large...")
    model_path = os.path.join(models_dir, "hetero_sage_model_approach5_large.pt")
...
def sweep_thresholds(lower_bound=0.1, upper_bound=0.95, step=0.05):
    ...
    results_dir_path = os.path.join(results_dir, "approach5_large_threshold_sweep.txt")
