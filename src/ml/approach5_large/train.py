...
from ml.approach5_large.model import HeteroSAGE
from ml.approach5_large.data import load_dataset, load_validation_dataset, get_negative_samples
...
hidden_channels = 256
batch_size = 8192 * 2
learning_rate = 0.001  # Lower from 0.01
dropout_edge_prob = 0.2  # Slightly reduce from 0.3
...
def ordinal_regression_loss(logits, targets):
    ...
    bce2 = F.binary_cross_entropy(probs[:, 1], targets[:, 1].float()) * 2 
    bce3 = F.binary_cross_entropy(probs[:, 2], targets[:, 2].float()) * 1.5
...
# Checkpoint loading
checkpoint_path = os.path.join(models_dir, "hetero_sage_model_approach5_large.pt")  # Updated path for approach5_large
start_epoch = 0
...
if os.path.exists(checkpoint_path):
    ...
    # Load pretrained weights from approach5 (excluding decoder)
    pretrained_checkpoint_path = os.path.join(models_dir, "hetero_sage_model_approach5.pt")
    if os.path.exists(pretrained_checkpoint_path):
        print(f"\nLoading pretrained weights from: {pretrained_checkpoint_path}")
        model.load_pretrained_weights(pretrained_checkpoint_path, device=device)
    else:
        print(f"\nWarning: Pretrained checkpoint not found at {pretrained_checkpoint_path}")
...
for epoch in range(start_epoch, num_epochs):
    ...
    if (epoch + 1) % checkpoint_interval == 0:
        checkpoint_path = os.path.join(models_dir, "hetero_sage_model_approach5_large.pt")
        torch.save({
            'epoch': last_epoch,
...
save_path = os.path.join(models_dir, "hetero_sage_model_approach5_large.pt")
torch.save({
