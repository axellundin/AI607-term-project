...
from ml.approach3.model import HeteroGAT, HeteroSAGE
from ml.approach3.data import load_dataset, load_validation_dataset, get_negative_samples
...
# Checkpoint loading
checkpoint_path = os.path.join(models_dir, "hetero_sage_model_approach3_larger.pt")  # or specify your checkpoint path
start_epoch = 0
...
for epoch in range(start_epoch, num_epochs):
    ...
    if (epoch + 1) % checkpoint_interval == 0:
        checkpoint_path = os.path.join(models_dir, "hetero_sage_model_approach3.pt")
        torch.save({
            'epoch': last_epoch,
...
save_path = os.path.join(models_dir, "hetero_sage_model_approach3.pt")
torch.save({
