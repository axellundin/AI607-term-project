...
from ml.task2_gnn.data import load_dataset, load_validation_dataset, get_negative_samples
...
def train():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i in tqdm(range(0, len(train_pairs), batch_size), desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            print("batch label shape : ",batch_labels[0] )

            loss = criterion(logits, batch_labels)
            
            print(f"Loss for epoch {epoch} : {loss:.6f}")
