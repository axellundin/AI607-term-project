...
def get_negative_samples(all_interactions, user2idx, item2idx, task1_users, N):
    """
    Generate negative samples by sampling from two sources:
    1. Save (2) and buy (3) interactions from task1 users
    2. No-interaction pairs (user-item pairs with no existing interaction)
    
    These are treated as negative samples (label 0) for task2 prediction 
    (which predicts view interactions).
    ...
    """
    # Collect all save (2) and buy (3) interactions from task1 users
    task1_save_buy_interactions = []
    for (user_id, item_id), interaction in all_interactions.items():
        if user_id in task1_users and interaction in [2, 3]:  # save or buy
            task1_save_buy_interactions.append((user_id, item_id))
    
    # Prepare for sampling no-interaction pairs
    ...
    # Sample half from save/buy interactions, half from no-interaction pairs
    # (or adjust proportionally if one source is insufficient)
    n_from_save_buy = min(N // 2, len(task1_save_buy_interactions))
    n_from_no_interaction = N - n_from_save_buy
    
    # Sample from save/buy interactions
    if len(task1_save_buy_interactions) > 0 and n_from_save_buy > 0:
        if n_from_save_buy <= len(task1_save_buy_interactions):
            sampled_indices = np.random.choice(len(task1_save_buy_interactions), size=n_from_save_buy, replace=False)
        else:
            sampled_indices = np.random.choice(len(task1_save_buy_interactions), size=n_from_save_buy, replace=True)
        
        for idx in sampled_indices:
            user_id, item_id = task1_save_buy_interactions[idx]
            negative_labels[(user_id, item_id)] = 0
            num_negative_samples += 1
    
    # Sample from no-interaction pairs
    attempts = 0
    max_attempts = n_from_no_interaction * 100  # Prevent infinite loop
    while num_negative_samples < N and attempts < max_attempts:
        user_id = task1_user_list[np.random.choice(num_task1_users)]
        item_id = item_id_lst[np.random.choice(num_items)]
        
        # Skip if this pair already exists in interactions or already sampled
        if (user_id, item_id) in all_interactions or (user_id, item_id) in negative_labels:
            attempts += 1
            continue
