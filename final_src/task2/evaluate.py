...
from task2.data_stage2 import build_gt_dict, evaluate
...
def gnn_recommend_batch(model, data, user_indices, num_items, topk, device, user_interacted_dict=None, batch_size=100):
    ...
    with torch.no_grad():
        ...
        for batch_start in tqdm(range(0, len(user_indices), batch_size), desc="Processing user batches", total=num_batches):
            ...
            logits = model(data, user_indices_expanded, item_indices_expanded)  # [num_batch_users * num_items, 3]
            logits,_= torch.max(logits, dim=-1) # [num_batch_users * num_items, 3]
...
def main():
    ...
    parser.add_argument("--input-path", type=str, default="hetero_sage_model_task2_prediction.pt")
    ...
    out_path = os.path.join(data_dir, "task2_test_answers.tsv")
    save_submission(predictions, val_users, idx2item, out_path, topk=topk)
