...
We implemented an apporoach based on representation learning combined with Heterogenous GNNs to solve this, and for the second task we accomplished domain transfer by utilizing the dataset for the first task to learn embeddings, that could then be used for predictions for the second user partition. Our approaches achieved 0.5139 Macro-F1 score on the validation set for the first task, and a 0.195 N-DCG score on the validation set for the second task.